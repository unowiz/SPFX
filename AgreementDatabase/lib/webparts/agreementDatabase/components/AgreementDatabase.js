var __extends = (this && this.__extends) || (function () {
    var extendStatics = Object.setPrototypeOf ||
        ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
        function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
import * as React from 'react';
import AgreementContainer from "../../../container/AgreementContainer";
import AgreementDataProvider from "../../../dataprovider/AgreementDataProvider";
var AgreementDatabase = (function (_super) {
    __extends(AgreementDatabase, _super);
    function AgreementDatabase(props) {
        return _super.call(this, props) || this;
    }
    AgreementDatabase.prototype.render = function () {
        var provider = new AgreementDataProvider(this.props.context.pageContext.web.absoluteUrl, this.props.context);
        return (React.createElement(AgreementContainer, { provider: provider }));
    };
    return AgreementDatabase;
}(React.Component));
export default AgreementDatabase;
//# sourceMappingURL=AgreementDatabase.js.map